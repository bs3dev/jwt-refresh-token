...
{
    public static async Task WriteAsync(this HttpContext httpContext, Jwt.Refresh.Token.Domain.DataTransferObjects.Token token)
    {
        ...
        {
            await httpContext.Response.Body.WriteAsync(JsonSerializer.SerializeToUtf8Bytes(
                token,
                AppJsonContext.Default.Token 
            ));
        }
    }
}